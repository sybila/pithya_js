...
function zoomed() {
  ...
  console.log(zoomObject);
  xScale = zoomObject.rescaleX(xScale);
  yScale = zoomObject.rescaleY(yScale);
  ...
  for(var v = 0; v < window.bio.vars.length; v++) {
    ...
    if(xDim == key)
      ...
      content += "\n"+xDim+": ["+(+parseFloat(d.x).toFixed(3)+", "\
                                +(+parseFloat(d.x+d.width).toFixed(3)+"]";
    else if(yDim == key)
      content += "\n"+yDim+": ["+(+parseFloat(d.y+d.height).toFixed(3)+", "\
